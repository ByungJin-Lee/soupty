...
pub fn calculate_active_chatter_ranking(
    ...
    _donations: &VecDeque<EnrichedDonationData>,
) -> ActiveChatterRankingData {
    let standard = Utc::now();
    let cutoff_time = standard - Duration::minutes(2);

    let mut chat_counts: HashMap<String, (String, u32)> = HashMap::new();

    for chat in chats.iter() {
        if chat.timestamp >= cutoff_time {
            let user_id = &chat.user.id;
            let nickname = &chat.user.label;
            
            chat_counts
                .entry(user_id.clone())
                .and_modify(|(_, count)| *count += 1)
                .or_insert((nickname.clone(), 1));
        }
    }

    let mut rankings: Vec<ChatterRankingItem> = chat_counts
        .into_iter()
        .map(|(user_id, (nickname, chat_count))| ChatterRankingItem {
            user_id,
            nickname,
            chat_count,
        })
