...
use crate::{
    controllers::{
        ...
        constants::{METADATA_UPDATE_TASK, MAIN_CONTROLLER_SUBSCRIBER},
    },
    services::{
        addons::{
...
impl SystemInitializer {
    ...
    pub async fn initialize_metadata_manager(
        ...
        metadata_manager.notify_initial_metadata(move |metadata: &BroadcastMetadata| {
            ...
            async move {
                ...
                event_bus_clone.publish(SystemEvent::MetadataUpdated {
        ...
        self.scheduler.schedule_recurring(METADATA_UPDATE_TASK, metadata_update_duration(), move || {
            ...
            async move {
                ...
                match metadata_manager.update_metadata(move |metadata: &BroadcastMetadata| {
                    let event_bus_clone = event_bus.clone();
                    let metadata_clone = metadata.clone();
                    ...
                    Err(error) => {
                        // 오류가 난 경우 방송 종료로 간주함.
                        event_bus_for_error.publish(SystemEvent::MetadataFetchFailed(error.to_string())).await;
                    }
                }
            }
        }).await;
